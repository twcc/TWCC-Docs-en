...
# HowTo: Use multi-node parallel computing－AI Benchmark 


:::success
<i class="fa fa-star" aria-hidden="true"></i> **Scenario: How to run more than 8 GPUs? How to create TAIWANIA 2 containers? How to submit jobs?**
...
Since `NGC 19.11` <sup>[2]</sup> version of the container, it has [experimental support for Singularity container](https://developer.nvidia.com/blog/how-to-run-ngc-deep-learning-containers-with-singularity/), and it is suitable for HPC usage scenarios. So, users of TAIWANIA 2 (HPC CLI) can directly download the NGC container for use, or use the container from NGC as the basic image to customize your common tools<sup>[3]</sup>.
...
If you want to use other versions or other containers, TAIWANIA 2 has already installed Singularity, you may run `singularity pull` command to download Singularity container <sup>[4]</sup>.
...
- After starting the computing, you can run the following command to view the log.
