...
:::success
<i class="fa fa-star" aria-hidden="true"></i> **適用情境：如何建立台灣杉二號的容器？如何客製化容器？**
...
:::info
<i class="fa fa-paperclip fa-20" aria-hidden="true"></i> **附註：** [<ins>Singularity</ins>](https://sylabs.io/) 工具相當於 Docker，可建立 Rootless 形式的容器。
:::

## 準備工作：容器映像檔

您可以下載容器映像檔，或客製化您的容器環境。

### 1. 下載容器映像檔

登入 TWCC 台灣杉二號環境後，您可以直接使用 Singularity `pull` 指令下載 Docker Hub 或 其他 Container Registry 上的容器映像檔，檔案將會自動轉成 Singularity SIF 格式。

例：下載 NVIDIA 在 Docker Hub 提供的 [CUDA 10.1 容器](https://hub.docker.com/r/nvidia/cuda/tags)：

```bash
$ singularity pull docker://nvidia/cuda:10.1-devel-ubuntu18.04
```
例：下載 NVIDIA 在 NGC Container Registry 提供的 [CUDA 10.1 容器](https://ngc.nvidia.com/catalog/containers/nvidia:cuda/tags)：

```bash
$ singularity pull docker://nvcr.io/nvidia/cuda:10.1-devel-ubuntu18.04
```
:::info
<i class="fa fa-paperclip fa-20" aria-hidden="true"></i> **附註：** 本文將使用 [<ins>NGC Container Registry</ins>](https://ngc.nvidia.com/catalog/containers) 提供的映像檔操作。
:::

### 2. 客製化 Singularity 容器
