...
When the container state shows **`Ready`**, you can start using it. Interactive Container provides Jupyter Notebook development environment and SSH to launch. Please refer to the following instructions for the process steps.

{%hackmd @twccdocs/chunk-ccs-prerequisite-id-en %}

## Launch the container

You can launch the container using Jupyter Notebook or SSH.
...
- **Step 2.** Then click "**New**" > "**Terminal**"  to launch the container environment. Then you can start artificial intelligence computing in the container.
...
- **Step 2.** Click "**URL**" (CLI) then you are able to open Jupyter Notebook.

![](https://cos.twcc.ai/SYS-MANUAL/uploads/upload_619c5fad19ccb469b5368895935ae48b.png)


- **Step 3.** Click "**New**" > "**Terminal**" to launch the container environment.
...
- **Step 1.** Click the "**Copy icon**" on the right of SSH to copy the supercomputer account and IP address.

![](https://cos.twcc.ai/SYS-MANUAL/uploads/upload_a64c20b74d97d7a2fdc4c2d2d2f05e2e.png)


- **Step 2.** Open Terminal in the local device, then fill in the relevant information for SSH login. The login account and password are the supercomputer account and password set in iService. You can also use other third-party software such as Putty to launch.
...
- **Step 1.** Get the SSH launch information of the container whose container ID is **`1249374`**
...
- Exit the container
    Insert **`exit`** then you are able to exit the container SSH environment.
