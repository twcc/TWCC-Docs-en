---
title: 使用 TWCC COS 備份/同步 還原運算資料 |en
tags: Guide, CCS, EN
GA: UA-155999456-1
---
...
- S3cmd is a command line tool that applicable for VCS instance(Linux) and CCS container. [<ins>Learn more about s3cmd</ins>](https://s3tools.org/s3cmd)
- Duplicati is a graphical user interface tool that applicable for VCS instance(Windows). [<ins>Learn more about Duplicati</ins>](https://duplicati.readthedocs.io/en/latest/01-introduction/)

## Preparation

- Create VCS instance or Interactive Container
- Create Cloud Object Storage(COS) to back up or synchronize your data
- Connect to your VCS instance or Interactive container, and obtain the folder/file path to be backed up/synchronized

## s3cmd: applicable for VCS instance(Linux) and CCS container
...
1. You can get shared COS connection information by clicking "SERVICES"> "Cloud Object Storage (COS)"> Third-party Software page.
...
2. Private COS connection information can be obtained by clicking "Private COS"> Third-party Software page.
...
```
**10. Use HTTPS protocol, enter```Yes``` (Be aware of the case of letters!)**
```shell
...
```
**12. Double check the setting content**
```shell
New settings:
...
After the setting is completed, you can start to store the VCS instances and containers data to COS bucket. There are two methods: backup and synchronization.


<span class = "number">&#10102;</span> **Backup**: All your data will be stored. If the data is deleted or lost by mistake, the backup data stored in COS can be restored at any time.
...
- **Back up data**
    ...
    Enter the following command to edit crontab job
    ```bash
    $ crontab -e 
    ```
...
- **Synchronize data**
    ...
    :::info
    <i class="fa fa-paperclip fa-20" aria-hidden="true"></i> **Note：** 
    Schedule time setting method:<b style="font-face:bold">* * * * *</b> (**Minute Hour Day     Month Week**)
    ...
    :::info
    <i class="fa fa-paperclip fa-20" aria-hidden="true"></i> **Note:** If you need to back up multiple versions of files, please enable the Bucket Versioning function. See [How to Enable or Disable Versioning for a Bucket](https://s3browser.com/amazon-s3-versioning.aspx) from S3 Browser.
    :::

### Recover backed up/synchronized files from COS

If the data of VCS instances/containers is deleted by mistake or is lost, you can restore the data. There are two methods to restore: backup restore and synchronization restore.

<span class = "number">&#10102;</span> **Backup restore**: All data backed up in COS will be restored to supercomputer, and the inconsistent data between both sides will still be stored in supercomputer.
<span class = "number">&#10103;</span> **Synchronization restore:** The data synchronized with COS will be completely restored to supercomputer, and the inconsistent data between both sides will be accorded with that in COS.
...
In addition to backing up, synchronizing and recovering data, s3cmd can also perform general operations on COS bucket:
...
- Delete a large number of files in the bucket (For xample: all files under the folder `gpu-burn`)
```bash=
$ s3cmd del s3://<BUCKET_NAME>/gpu-burn/*
```
- Delete the folder in the bucket
```bash=
...
```
:::info
<i class="fa fa-paperclip fa-20" aria-hidden="true"></i> **Note:** [More commands operating s3cmd](https://s3tools.org/usage)
:::
...
## Duplicati: applicable for VCS instance(Windows)
...
After the installation, you can start to store the VCS instances and containers data to COS bucket.
...
- Bucket name: enter a name for backup bucket
...
For example: regular backup automatically at 13:00 every day
...
Select remote volume size as the default value (if your Internet speed is low, you can select a lower size). Select backup retention according to your needs (*keep all backups, delete backups that are older than a specific time, keep a specific number of backups, smart backup retention or custom backup retention.*).
...
- If you don’t need to back up data anymore, please click "Delete" to delete the configuration
...
After the restoration, it will show *Your files and folders have been restored successfully* message.
